# -*- coding: utf-8 -*-
"""Proceso_Completo_Uber.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Dt56eVWu0SDgrORE_yp76lp3CtcJ1Xsg

# 🚀 Uber Riders — Proceso Automatizado Completo (Bronze ➔ Silver ➔ Dashboard)

Este cuaderno unifica todo el pipeline de datos:
1. **Bronze**: Lee los CSVs crudos de Uber y consolida el histórico.
2. **Silver**: Deduplica quedándose con la versión más reciente por (rider, día, market).
3. **Dashboard**: Genera el HTML interactivo y lo sube a Google Drive.
"""
...
UBER_FOLDER_ID = '1PNsRT6zCBmOFLqLqhvWqis68Ih0VpxyY'
OUTPUT_FOLDER_NAME = 'Uber Riders TEST'
MIN_FILE_DATE = '2026-03-13'
FILE_PATTERN = re.compile(r'^COURIER_DAILY_CLOSERLOGISTICS_\d{8}_\d{6}\.csv$')

BRONZE_FILENAME = 'rides_raw.parquet'
SILVER_FILENAME = 'rides_silver.parquet'
HTML_FILENAME = 'dashboard.html'

# Columnas que van al HTML. Se incluyen ~29 columnas de las 37 del Silver
# para que el tab 'Datos en bruto' tenga todo lo útil.
DASH_COLS = [
    'datestr', 'weekstr',
    'driver_uuid', 'driver_name', 'driver_email', 'driver_number',
    'city_name', 'market_name', 'form_factor',
    'online_hours', 'active_hours', 'open_hours',
    'enroute_p2_hours', 'ontrip_p3_hours', 'unavailable_hours',
    'num_of_trips', 'single_trips_total',
    'late_p2_trips', 'late_p3_trips',
    'accept_trips', 'reject_trips', 'cancel_trips', 'cancel_not_at_fault_trips',
    'p2_km', 'p2_min', 'p3_km', 'p3_min',
    'total_km', 'total_min',
]

# Factorizar strings muy repetidos + fechas para reducir tamaño del JSON
FACTORIZE = {'driver_uuid','driver_name','driver_email','driver_number',
             'city_name','market_name','form_factor','datestr','weekstr'}

# Patrón del nombre de los CSVs
import re
FILE_PATTERN = re.compile(r'^COURIER_DAILY_CLOSERLOGISTICS_\d{8}_\d{6}\.csv$')

# Nombre del fichero Parquet consolidado
PARQUET_FILENAME = 'rides_raw.parquet'

print('Configuración cargada.')

"""# Uber Riders — Ingesta Bronze

Este notebook lee los CSVs que Uber sube a una carpeta de Drive y los consolida en un único Parquet (capa **Bronze**: sin deduplicar, todo crudo).

Cada fila se enriquece con dos columnas:
- `file_date` → timestamp del fichero de origen (extraído del nombre).
- `file_name` → nombre del fichero de origen.
"""

def find_or_create_folder(name, parent_id=None):
    """Busca una carpeta con ese nombre en tu Drive personal. Si no existe, la crea."""
    query_parts = [
        f"name = '{name}'",
    ...
    if parent_id:
        ...
        # Solo carpetas propias (no compartidas conmigo)
...
OUTPUT_FOLDER_ID = find_or_create_folder(OUTPUT_FOLDER_NAME)

"""## Listar los CSVs disponibles en la carpeta de Uber

Filtra por patrón de nombre y por fecha mínima.
"""

from datetime import datetime

def extract_ts(name):
    """De 'COURIER_DAILY_CLOSERLOGISTICS_20260422_071233.csv' → datetime."""
    m = re.search(r'(\d{4})(\d{2})(\d{2})_(\d{2})(\d{2})(\d{2})', name)
    if not m:
        return None
    return datetime(*(int(x) for x in m.groups()))

def list_uber_csvs(folder_id, min_date=None):
    """Lista todos los CSVs de la carpeta, quedándose solo con el último de cada día."""
    files = []
    page_token = None
    while True:
        resp = drive.files().list(
            ...
            supportsAllDrives=True,          # Para asegurar que ve unidades compartidas
            includeItemsFromAllDrives=True   # Para asegurar que ve unidades compartidas
        ).execute()
    ...
    # --- NUEVA LÓGICA: Filtrar el más reciente por día ---
    archivos_por_dia = {}
    for f in valid:
        # Extraemos solo la fecha (año, mes, día), ignorando horas/minutos
        dia = f['timestamp'].date()

        # Si es la primera vez que vemos este día, o si el archivo actual
        # tiene un timestamp mayor (es más reciente) que el que ya teníamos guardado:
        if dia not in archivos_por_dia or f['timestamp'] > archivos_por_dia[dia]['timestamp']:
            archivos_por_dia[dia] = f

    # Extraemos solo los archivos ganadores del diccionario
    archivos_finales = list(archivos_por_dia.values())

    # Ordenamos de más antiguo a más reciente para procesarlos en orden lógico
    archivos_finales.sort(key=lambda f: f['timestamp'])

    return archivos_finales

csvs = list_uber_csvs(UBER_FOLDER_ID, min_date=MIN_FILE_DATE)
print(f"CSVs únicos (1 por día) encontrados: {len(csvs)}")
if csvs:
    print(f"Rango de timestamps: {csvs[0]['timestamp']} → {csvs[-1]['timestamp']}")
    print(f"Primeros 3: {[f['name'] for f in csvs[:3]]}")
    print(f"Últimos 3: {[f['name'] for f in csvs[-3:]]}")

"""## Determinar qué CSVs son nuevos

Si ya existe un Parquet previo, leemos qué ficheros están ya dentro (columna `file_name`) y solo procesamos los nuevos. Si es la primera ejecución, se procesan todos.
"""
...
def download_drive_file(file_id):
    """Descarga un fichero de Drive y devuelve su contenido como bytes."""
...
existing_parquet = find_file_by_name(PARQUET_FILENAME, OUTPUT_FOLDER_ID)
processed_files = set()
existing_df = None

if existing_parquet:
    print(f"Parquet previo encontrado ({existing_parquet['id']}). Descargando para ver qué ficheros ya están procesados...")
    buf = download_drive_file(existing_parquet['id'])
    existing_df = pl.read_parquet(buf)
    processed_files = set(existing_df['file_name'].unique().to_list())
    print(f"  Filas en Parquet previo: {len(existing_df):,}")
    print(f"  Ficheros ya procesados: {len(processed_files)}")
else:
    print('No hay Parquet previo. Primera ejecución.')

new_csvs = [f for f in csvs if f['name'] not in processed_files]
print(f"\nCSVs nuevos a procesar: {len(new_csvs)} (de {len(csvs)} totales)")

"""## Descargar y parsear los CSVs nuevos con Polars

Cada CSV se parsea tolerando schemas antiguos (columnas faltantes se rellenan con null). Se añaden `file_date` y `file_name`.
"""

# Esquema canónico (el más reciente). Todo CSV se promociona a este schema.
# Polars es tolerante: columnas que no existan en el CSV quedan como null.
CANONICAL_COLUMNS = [
    'weekstr', 'datestr',
    'driver_uuid', 'driver_name', 'driver_number', 'driver_email',
    ...
    'late_p2_trips', 'late_p3_trips',
    'accept_trips', 'reject_trips', 'cancel_trips', 'cancel_not_at_fault_trips',
    'p2_km', 'p2_min', 'p2_km_avg', 'p2_min_avg',
    'p3_km', 'p3_min', 'p3_km_avg', 'p3_min_avg',
    'total_km', 'total_min', 'total_km_avg', 'total_min_avg',
]

# Tipos por columna (polars los infiere bien, pero forzamos para consistencia)
TEXT_COLS = {'driver_uuid', 'driver_name', 'driver_number', 'driver_email',
             'fleet_name', 'city_name', 'market_name', 'form_factor'}
DATE_COLS = {'weekstr', 'datestr'}
# El resto son numéricas.

def parse_csv(content: bytes, file_name: str, file_ts: datetime) -> pl.DataFrame:
    """Lee bytes de CSV → DataFrame promocionado al schema canónico."""
    # Lectura tolerante: infer_schema_length alto para detectar bien los tipos numéricos
    df = pl.read_csv(
        io.BytesIO(content),
    ...
    # Añadir columnas que falten en este CSV (null) al esquema canónico
    ...
    # Quitar columnas extra que no estén en el esquema canónico
    df = df.select(CANONICAL_COLUMNS)

    # Parsear columnas de fecha (vienen como '2026-04-15 00:00:00.000')
    for col in DATE_COLS:
        df = df.with_columns(
            pl.col(col).str.to_datetime(strict=False).alias(col)
        )

    # Forzar tipos numéricos (columnas que quedaron como string por tener muchos nulls)
    for col in CANONICAL_COLUMNS:
        if col not in DATE_COLS and col not in TEXT_COLS:
            df = df.with_columns(pl.col(col).cast(pl.Float64, strict=False))

    # Añadir metadata del fichero
    df = df.with_columns([
...
for i, f in enumerate(new_csvs, 1):
    try:
        print(f"[{i}/{len(new_csvs)}] {f['name']} ({int(f.get('size', 0))/1024:.0f} KB)...", end=' ')
        buf = download_drive_file(f['id'])
        df = parse_csv(buf.read(), f['name'], f['timestamp'])
        new_dfs.append(df)
        print(f"{len(df):,} filas")
    except Exception as e:
        print(f"ERROR: {e}")

print(f"\nTotal DataFrames nuevos: {len(new_dfs)}")

"""## Concatenar con el histórico previo"""

if new_dfs:
    new_all = pl.concat(new_dfs, how='vertical_relaxed')
    print(f"Filas nuevas: {len(new_all):,}")
else:
    new_all = None
    print('No hay filas nuevas.')

if existing_df is not None and new_all is not None:
    final_df = pl.concat([existing_df, new_all], how='vertical_relaxed')
    ...
    final_df = None
    print('Nada que consolidar. Saliendo.')

if final_df is not None:
    print(f"\nFilas totales tras concatenar: {len(final_df):,}")
    print(f"Columnas: {len(final_df.columns)}")

"""## Subir (o sobrescribir) el Parquet en Drive"""

if final_df is not None:
    # Escribir a buffer
    buf = io.BytesIO()
    final_df.write_parquet(buf, compression='zstd')
    buf.seek(0)
    size_kb = buf.getbuffer().nbytes / 1024
    print(f"Parquet generado: {size_kb:.1f} KB")

    media = MediaIoBaseUpload(buf, mimetype='application/octet-stream', resumable=False)
    if existing_parquet:
        updated = drive.files().update(
            fileId=existing_parquet['id'], media_body=media,
        ).execute()
        print(f"Parquet actualizado en Drive: {PARQUET_FILENAME} (id: {updated['id']})")
    else:
        created = drive.files().create(
            body={'name': PARQUET_FILENAME, 'parents': [OUTPUT_FOLDER_ID]},
            media_body=media,
            fields='id, name, webViewLink',
        ).execute()
        print(f"Parquet creado en Drive: {created['name']} (id: {created['id']})")
        print(f"Link: {created.get('webViewLink', '(no disponible)')}")
else:
    print('Nada que subir.')

"""## Exploración rápida

Vistazo para validar que los datos esten correctos.
"""
...
if final_df is not None:
    ...
    print()
    print('=== Rango de datestr ===')
    print(f"  {final_df['datestr'].min()} → {final_df['datestr'].max()}")
    print()
    print('=== Rango de file_date ===')
    print(f"  {final_df['file_date'].min()} → {final_df['file_date'].max()}")
    print()
    print('=== Ficheros procesados ===')
    print(f"  {final_df['file_name'].n_unique()} únicos")
    print()
    print('=== Filas por file_date (top 10 más recientes) ===')
    print(
        final_df.group_by('file_name', 'file_date').len()
                .sort('file_date', descending=True).head(10)
    )
    print()
    print('=== Primeras 3 filas ===')
    print(final_df.head(3))

"""# Uber Riders — Capa Silver (deduplicación)

Lee el Parquet **Bronze** (todas las filas, todos los ficheros, sin deduplicar) y produce el Parquet **Silver** aplicando la regla consensuada:

> **Para cada `(driver_uuid, datestr)`**, mirar en qué `file_date` aparece con valor más reciente. De **ese fichero** quedarse con **todas** las filas que tenga ese rider ese día (una por cada market). Descartar lo que venga de ficheros anteriores para esa combinación.

Esto respeta que:
- La versión más reciente de los datos es la que vale (datos que maduran).
- Si un rider trabajó legítimamente en 2 markets un día, conservamos las 2 filas.
- Si Uber consolidó múltiples filas en una, nos quedamos con la consolidada (no hay doble conteo).

**Nota sobre madurez**: este notebook NO aplica el filtro `datestr <= hoy - 2 días`. Silver contiene todo, y el filtro de madurez se aplica después en el dashboard para poder modificarlo sin recomputar Silver.

## Localizar la carpeta y el Parquet Bronze
"""
...
def find_folder(name):
    ...
    files = resp.get('files', [])
    if not files:
        raise RuntimeError(f"No encuentro la carpeta '{name}' en tu Drive. Ejecuta primero el notebook Bronze.")
    return files[0]['id']
...
# 1. Verificamos que la fase Bronze haya pasado datos válidos
if 'final_df' not in locals() or final_df is None:
    raise RuntimeError("No hay datos de la fase Bronze. Asegúrate de hacer clic en 'Ejecutar todas las celdas' y verifica que existan CSVs para la fecha configurada.")

# 2. Pasamos los datos directamente por la memoria RAM
bronze = final_df

# 3. Imprimimos el resumen
print(f"Bronze cargado: {len(bronze):,} filas × {len(bronze.columns)} columnas")
print(f"Rango datestr: {bronze['datestr'].min()} → {bronze['datestr'].max()}")
print(f"Rango file_date: {bronze['file_date'].min()} → {bronze['file_date'].max()}")
print(f"Ficheros distintos: {bronze['file_name'].n_unique()}")
print(f"Drivers únicos: {bronze['driver_uuid'].n_unique():,}")

"""## Aplicar la regla de deduplicación (enfoque A)

**Regla**: para cada `(driver_uuid, datestr)`, mirar cuál es el `file_date` máximo que tenga filas para ese par, y quedarse con todas las filas de ese fichero para ese par. Esto respeta multi-market (si el último fichero trae 2 filas del mismo rider-día, nos quedamos con las 2).
"""

# 1. Normalizar la fecha al nivel de día (datestr viene con hora 00:00:00 pero por si acaso)
#    Trabajamos con la columna tal cual.

# 2. Para cada (driver, datestr), calcular el file_date más reciente
silver = (
    bronze
    .with_columns(
        pl.col('file_date').max().over(['driver_uuid', 'datestr']).alias('_max_file_date')
    )
    .filter(pl.col('file_date') == pl.col('_max_file_date'))
    .drop('_max_file_date')
)

# 3. Edge case: si una fila tiene datestr null (no debería, pero por seguridad), descartarla
silver = silver.filter(pl.col('datestr').is_not_null() & pl.col('driver_uuid').is_not_null())

print(f"Bronze → Silver: {len(bronze):,} → {len(silver):,} filas")
print(f"Reducción: {(1 - len(silver)/len(bronze))*100:.1f}%")

"""## Validaciones

Comprobaciones que deben cumplirse si la lógica es correcta.
"""

# 6.1. Cada (driver, día, market) aparece UNA sola vez en Silver
dupes = (
    silver.group_by(['driver_uuid', 'datestr', 'market_name']).len()
          .filter(pl.col('len') > 1)
)
print(f"Tripletas (driver, día, market) con >1 fila en Silver: {len(dupes)}")
if len(dupes) > 0:
    print('  ⚠️  No debería haber. Mirar primeras 5:')
    print(dupes.head(5))
else:
    print('  ✓ OK: sin duplicados a nivel (driver, día, market)')

# 6.2. Todas las filas de un mismo (driver, día) vienen del mismo file_name
multi_file = (
    silver.group_by(['driver_uuid', 'datestr'])
          .agg(pl.col('file_name').n_unique().alias('n_files'))
          .filter(pl.col('n_files') > 1)
)
print(f"\nPares (driver, día) que mezclan varios ficheros en Silver: {len(multi_file)}")
if len(multi_file) > 0:
    print('  ⚠️  No debería pasar con enfoque A. Mirar primeras 5:')
    print(multi_file.head(5))
else:
    print('  ✓ OK: cada (driver, día) viene de un solo fichero')

"""## Mini-exploración

Vistazo a los datos ya limpios.
"""

# Distribución por día
by_day = (
    silver.group_by(silver['datestr'].dt.date().alias('day'))
          .agg([
              pl.len().alias('rows'),
              pl.col('driver_uuid').n_unique().alias('drivers'),
              pl.col('num_of_trips').sum().alias('trips'),
              pl.col('total_km').sum().alias('km'),
          ])
          .sort('day')
)
print('=== Filas por día ===')
...
# Markets más activos
print('\n=== Top 10 markets por trips ===')
print(
    silver.group_by('market_name')
          .agg([pl.len().alias('rows'), pl.col('num_of_trips').sum().alias('trips')])
          .sort('trips', descending=True)
          .head(10)
)

"""## Exploración opcional: datos madurando

Para cada (driver, día) mira **cuántas versiones diferentes del valor `num_of_trips` había en Bronze**. Si hay valores > 1, significa que ese rider-día tuvo datos que fueron cambiando entre ficheros hasta estabilizarse.

Útil para decidir en qué días la madurez todavía no es completa.
"""

# Cuántas versiones distintas de num_of_trips hay por (driver, día, market)
maturation = (
    bronze
    .group_by(['driver_uuid', 'datestr', 'market_name'])
    .agg([
        pl.col('num_of_trips').n_unique().alias('n_versions'),
        pl.col('file_date').min().alias('first_seen'),
        pl.col('file_date').max().alias('last_seen'),
    ])
    .filter(pl.col('n_versions') > 1)
)

print(f"Tripletas (driver, día, market) con valor cambiante entre ficheros: {len(maturation):,}")

if len(maturation) > 0:
    # Cuánto tiempo tardan en estabilizarse
    maturation = maturation.with_columns(
        (pl.col('last_seen') - pl.col('first_seen')).alias('maturation_span')
    )
    print(f"\nTiempo hasta estabilización:")
    print(f"  Media: {maturation['maturation_span'].mean()}")
    print(f"  Máximo: {maturation['maturation_span'].max()}")
    print(f"  Mediana: {maturation['maturation_span'].median()}")

    print('\nPrimeras 5 filas:')
    print(maturation.head(5))

"""## Guardar Silver en Drive"""

buf = io.BytesIO()
silver.write_parquet(buf, compression='zstd')
buf.seek(0)
size_kb = buf.getbuffer().nbytes / 1024
print(f"Silver Parquet: {size_kb:.1f} KB para {len(silver):,} filas")

existing_silver = find_file_by_name(SILVER_FILENAME, OUTPUT_FOLDER_ID)
media = MediaIoBaseUpload(buf, mimetype='application/octet-stream', resumable=False)

if existing_silver:
    drive.files().update(fileId=existing_silver['id'], media_body=media).execute()
    print(f"Silver actualizado: {SILVER_FILENAME}")
else:
    created = drive.files().create(
        body={'name': SILVER_FILENAME, 'parents': [OUTPUT_FOLDER_ID]},
        media_body=media,
        fields='id, webViewLink',
    ).execute()
    print(f"Silver creado: {SILVER_FILENAME}")
    print(f"Link: {created.get('webViewLink')}")

"""# Uber Riders — Dashboard HTML

Lee el Parquet **Silver** y construye un `dashboard.html` autocontenido con:

- **Resumen**: KPIs (actividad, eficiencia, calidad) + 4 gráficos diarios (trips, horas, TPH, % cancelación).
- **Por rider**: tabla buscable, ordenable, con panel lateral detallado por rider y export a Excel.
- **Por market**: tabla + gráfico de barras horizontal de todos los markets.
- **Evolución**: gráfico de líneas con métrica y agrupación configurables.
- **Datos en bruto**: tabla con todas las filas del Silver y export a Excel.

Sube el HTML a `Uber Riders Output`.

## Localizar Silver y carpeta de output
"""
...
def find_folder(name):
    ...
    files = resp.get('files', [])
    if not files:
        raise RuntimeError(f"No encuentro la carpeta '{name}'. Ejecuta primero el notebook Bronze.")
    return files[0]['id']
...
# 1. Verificamos que la fase Silver haya producido datos
if 'silver' not in locals() or silver is None:
    raise RuntimeError("No hay datos de la fase Silver. Asegúrate de hacer clic en 'Ejecutar todas las celdas'.")

# 2. (La variable 'silver' ya viene lista directamente de la memoria de la fase anterior)

# 3. Imprimimos el resumen
print(f"Silver: {len(silver):,} filas × {len(silver.columns)} columnas")
print(f"Rango: {silver['datestr'].min()} → {silver['datestr'].max()}")
print(f"Drivers únicos: {silver['driver_uuid'].n_unique():,}")
print(f"Markets únicos: {silver['market_name'].n_unique()}")

"""## Construir el payload columnar + factorizado + semanas

**Columnar + factorizado**: strings repetidos (UUIDs, emails, markets) se guardan una vez y se referencian por índice. Reduce el JSON ~85%.

**Semanas**: se generan todas las semanas ISO (lunes-domingo) presentes en los datos más las últimas 8 semanas hasta hoy, para el selector de semana.
"""
...
def generate_weeks(min_date, max_date, lookback_weeks=8):
    """Genera semanas ISO (lun-dom) desde min_date hasta hoy (o max_date si es mayor)."""
    today = date.today()
    end = max(max_date, today)
    # Incluir también semanas anteriores a min_date por si acaso (lookback)
    ...
    while cur <= end:
        ...
        iso = cur.isocalendar()
        weeks.append({
            'week_num': iso[1] if isinstance(iso, tuple) else iso.week,
            'year': iso[0] if isinstance(iso, tuple) else iso.year,
            'start': cur.isoformat(),
            'end': week_end.isoformat(),
        })
        cur += timedelta(days=7)
    # Ordenar del más reciente al más antiguo (más útil en el select)
    weeks.sort(key=lambda w: w['start'], reverse=True)
    return weeks

# Preparar df: seleccionar columnas disponibles + formatear fechas como YYYY-MM-DD
...
meta = {
    ...
    'n_drivers': silver['driver_uuid'].n_unique(),
    'date_min': silver['datestr'].min().strftime('%Y-%m-%dT%H:%M:%S'),
    'date_max': silver['datestr'].max().strftime('%Y-%m-%dT%H:%M:%S'),
    'markets': sorted([m for m in silver['market_name'].unique().to_list() if m]),
    'cities': sorted([c for c in silver['city_name'].unique().to_list() if c]),
    'form_factors': sorted([f for f in silver['form_factor'].unique().to_list() if f]),
    'weeks': weeks,
...
print(f"Data: JSON {len(data_raw)/1024:.1f} KB → gzip {len(data_gz)/1024:.1f} KB → base64 {len(data_b64)/1024:.1f} KB")
print(f"Meta: JSON {len(meta_raw)/1024:.1f} KB → gzip {len(meta_gz)/1024:.1f} KB → base64 {len(meta_b64)/1024:.1f} KB ({len(weeks)} semanas)")
print(f"Reducción total: {(1 - (len(data_b64)+len(meta_b64)) / (len(data_raw)+len(meta_raw)))*100:.1f}%")
print(f"Columnas incluidas: {len(available_cols)}")
...
print(f'Template cargado: {len(TEMPLATE_HTML)/1024:.1f} KB')
...
print(f'HTML generado: {len(html)/1024:.1f} KB ({len(html)/1024/1024:.2f} MB)')

"""## Subir HTML a Drive"""

html_buf = io.BytesIO(html.encode('utf-8'))
media = MediaIoBaseUpload(html_buf, mimetype='text/html', resumable=False)

existing_html = find_file_by_name(HTML_FILENAME, OUTPUT_FOLDER_ID)
if existing_html:
    updated = drive.files().update(fileId=existing_html['id'], media_body=media, fields='id, webViewLink').execute()
    print(f'HTML actualizado: {HTML_FILENAME}')
    print(f'Link: {updated.get("webViewLink")}')
else:
    created = drive.files().create(
        body={'name': HTML_FILENAME, 'parents': [OUTPUT_FOLDER_ID]},
        media_body=media,
        fields='id, webViewLink',
    ).execute()
    print(f'HTML creado: {HTML_FILENAME}')
    print(f'Link: {created.get("webViewLink")}')

"""## Generar CSV para cargarlo al dashboard de riders"""

# --- NUEVO: Exportar la capa Silver a CSV y subirlo a Drive ---

# 1. Convertir el DataFrame Silver a CSV en la memoria
csv_buf = io.BytesIO()
silver.write_csv(csv_buf)
csv_buf.seek(0)
size_mb = csv_buf.getbuffer().nbytes / (1024 * 1024)
print(f"CSV generado en memoria: {size_mb:.2f} MB")

# 2. Subir el CSV a la misma carpeta de Drive
CSV_FILENAME = 'rides_silver.csv'
csv_media = MediaIoBaseUpload(csv_buf, mimetype='text/csv', resumable=False)
existing_csv = find_file_by_name(CSV_FILENAME, OUTPUT_FOLDER_ID)

if existing_csv:
    drive.files().update(fileId=existing_csv['id'], media_body=csv_media).execute()
    print(f"CSV actualizado en Drive: {CSV_FILENAME}")
else:
    drive.files().create(
        body={'name': CSV_FILENAME, 'parents': [OUTPUT_FOLDER_ID]},
        media_body=csv_media,
        fields='id'
    ).execute()
    print(f"CSV creado en Drive: {CSV_FILENAME}")